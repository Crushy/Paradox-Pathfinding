...
public:
    ...
    ///
    /// The size of the \a pixel array must match the \a _width and
    /// \a _height arguments, and it must contain 32-bits RGBA pixels.
    ///
    /// No additional check is performed on the size of the pixel
    ...
    ///
    /// \param pixels Array of pixels to copy to the texture
    /// \param _width  Width of the pixel region contained in \a pixels
    /// \param _height Height of the pixel region contained in \a pixels
    ...
    /// outside the texture rectangle [0, 0, _width, _height].
    /// In this case, if repeat mode is enabled, the whole texture
    /// will be repeated as many times as needed to reach the
    /// coordinate (for example, if the X texture coordinate is
    /// 3 * _width, the texture will be repeated 3 times).
